    ...
    <div class="row mt-1">
        <div class="col-12">
            <div class="card p-4" style="background-color:#ffffff;box-shadow: rgba(50, 50, 93, 0.25) 0px 6px 12px -2px, rgba(0, 0, 0, 0.3) 0px 3px 7px -3px;">
                <h1 class="mt-2">Edit Admin</h1>
                <hr class=" " style="eight: 2px;">
            ...
            <div class="card p-4 mt-4" style="box-shadow: rgba(50, 50, 93, 0.25) 0px 13px 27px -5px, rgba(0, 0, 0, 0.3) 0px 8px 16px -8px;">
                <div class="row mt-1">
                    <form action="{{ route('admin.update',$admin->id) }}" method="post">
                        ...
                        <div class="form-group">
                            <div class="row">
                                <div class="col-6">
                                    ...
                                    <input type="text" class="form-control form-control-lg @error('name') is-invalid @enderror" id="name" placeholder="Nama" name="name" value="{{ $admin->name }}">
                                    @error('name')
                                    <div class="invalid-feedback">{{$message}}</div> @enderror
                                </div>
                                <div class="col-6">
                                    <label class="form-label">NIM</label>
                                    <input type="number" class="form-control form-control-lg @error('nim') is-invalid @enderror" id="nim" placeholder="NIM" name="nim" value="{{$admin->nim}}">
                                    @error('nim')
                                    <div class="invalid-feedback">{{$message}}</div> @enderror
                                </div>
                            </div>
                        ...
                        <div class="form-group mb-4">
                            <div class="row">
                                <div class="col-12">
                                    ...
                                    <input type="email" class="form-control form-control-lg @error('email') is-invalid @enderror" id="email" placeholder="nim@uksw.edu" name="email" value="{{$admin->email}}">
                                    @error('email')
                                    <div class="invalid-feedback">{{$message}}</div> @enderror
                                </div>
                            </div>
                                ...
                                <div class="col-12">
                                    ...
                                    <div class="input-group mb-3">
                                        <select class="form-select form-select-lg" id="inputGroupSelect01" name="role_id">
                                            ...
                                            <option value="{{$role->id }}" {{($admin->role_id == $role->id) ? 'selected': ''}}>{{$role->role}}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                </div>
                        ...
                        <hr class="mt-4 mb-4">
                        <button class="col-2 btn btn-md btn-dark" type="submit" name="submit" style="background:#003289">Edit
                        </button>
                    </form>
                </div>
            </div>
